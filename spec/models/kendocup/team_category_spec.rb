...
module Kendocup
  ...
  describe "A team_category team", type: :model do
    let!(:team_category) {create :kendocup_team_category, name: 'team', pool_size: 3, out_of_pool: 2, cup: create(:kendocup_cup, start_on: Date.parse("2016-09-28"))}
    ...
    context "with a pool of 3 participations" do
      ...
      let!(:participation2){create :kendocup_participation, category: team_category, pool_number: 1, pool_position: 2, kenshi: create(:kendocup_kenshi, cup: team_category.cup)}
      let!(:participation3){create :kendocup_participation, category: team_category, pool_number: 1, pool_position: 3, kenshi: create(:kendocup_kenshi, cup: team_category.cup)}
    ...
    context "with 24 participations" do
      before {
        24.times do |i|
          kenshi = create :kendocup_kenshi, first_name: "fn_#{i}", last_name: "ln_#{i}", cup: team_category.cup
          create :kendocup_participation, category_type: "Kendocup::TeamCategory", category_id: team_category.id, kenshi: kenshi
        end
